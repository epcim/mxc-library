...
#K8sMonitoring: schema.#AppCore & {
	...
	context: {
		...
		integrations: {
			...
			loki: instances: [{
				name: #Loki.appName
				labelSelectors: "app.kubernetes.io/name": #Loki.appName
			}]
			mimir: instances: [{
				name: #Mimir.appName
				labelSelectors: "app.kubernetes.io/name": #Mimir.appName
			}]
			grafana: instances: [{
				name: #Grafana.appName
				labelSelectors: "app.kubernetes.io/name": #Grafana.appName
			}]
		}
		destinations: [{
			name:     "hostedPrometheus"
			type:     "prometheus"
			url:      "http://mimir-nginx.\(#Mimir.kustomize.namespace).svc:80/api/v1/push"
			tenantId: "anonymous"
			headers: {
			...
			name:     "hostedLoki"
			type:     "loki"
			url:      "http://loki-gateway.\(#Loki.kustomize.namespace).svc:80/loki/api/v1/push"
			tenantId: "anonymous"
			headers: {
				"X-Scope-OrgID": "anonymous"
			}
